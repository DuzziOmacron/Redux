...
ominous_dots(bcolors.OKGREEN + ("Sobering up") + bcolors.ENDC) # This lets the user know the bot is booting up.
...
try:
  version_page = "https://pastebin.com/7f4Ak7br" #This grabs the page where the current version is
...
global username_thing, spam, debug_jid, thing, attempt_number, given_pass, setup_preset, jid_list, name #This declares these variables as global to be used everywhere because I'm lazy
...
time.sleep(1)
print(bcolors.FAIL + emoji.emojize(":warning: This color means you skid like shit.") + bcolors.ENDC) #This is an example red text, ooooo
time.sleep(3)
print(bcolors.OKBLUE + emoji.emojize("Okay, lets get this shit over with..") + bcolors.ENDC)
    ...
    class SpamBotnet(KikClientCallback):
        ...
        def on_group_message_received(self, chat_message: chatting.IncomingGroupChatMessage):
            ...
            if antipurge == True:
                ...
                if mssg == "promote":
                    self.client.send_chat_message(GJID, "To use the promote command, say \"promote [JID or username]\"")
                else:
                    jid_to_promote = mssg.replace("promote ", "")
                    if jid_to_promote.startswith("@"):
                        self.client.send_chat_message(GJID, "Reminder: You do not need to add an \"@\" to the username in this command.")
                ...
                if mssg == "demote":
                    ...
                    self.client.send_chat_message(GJID, "Attempting to demote \"" + jid_to_demote + "\"!")
        ...
        def on_roster_received(self, response: FetchRosterResponse): #This function is triggered when the roster is recieved.
            ...
            self.client.send_chat_message(debug_jid, emoji.emojize(":warning: \"" + name + "\" is online!\nHost: " + str(hostname) + "\nIP: " + str(ip_address)))
            for user in jid_list:
                self.client.add_friend(user)
                time.sleep(0.3)
        ...
        def on_connection_failed(self, response: ConnectionFailedResponse): #This function is triggered when your kik bot's connection is closed/fails
            ...
            print(bcolors.OKGREEN + emoji.emojize(" Kik was on some shit..") + bcolors.ENDC) #This lets the user know the connection was closed
...
def retry_login(password): #This function retries logns when there are closed connections
    ...
    if number_of_errors > 0: #This checks if there are any errors, if so, attempts to log in.
        print(bcolors.OKBLUE + emoji.emojize("There are " + str(number_of_errors) + " Fucked connections. Lemme try again..") + bcolors.ENDC) #This lets the user know that the script will retry the login(s)
        ominous_dots(bcolors.OKGREEN + emoji.emojize("This better work!") + bcolors.ENDC)
        print("\n")
        ...
        for name in retry_list: #This loops through the failed logins
            if number_of_errors > 0: #This checks if there are errors remaining before continuing
                ...
                time.sleep(18) #This waits 5 seconds between each login to avoid closed connections
...
number_of_attempts = 30 #This specifies the number of times the bot will retry logins when connections are closed. I recommend you leave it alone.
