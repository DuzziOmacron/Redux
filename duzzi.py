...
ominous_dots(bcolors.OKGREEN + ("toke'n up") + bcolors.ENDC) # This lets the user know the bot is booting up.
...
def get_bot_quantity(): #This function asks the user how many bots they wanna use.
    try:
        thing = int(input(bcolors.OKGREEN + ("\nHow many niggas in yo crew?: ") + bcolors.ENDC))
    except ValueError: #This is triggered when the input is not a number.
        print(bcolors.FAIL + ("Is you retarded, lol. try again nigga.") + bcolors.ENDC)
...
if use_preset == True:
    ...
    given_pass = input(bcolors.OKGREEN + ("\n" + str(thing) + " niggas, got it! What is the password for your bots?: ") + bcolors.ENDC) #This asks for the bot's password, it doesn't need error handling.
...
time.sleep(1)
print(bcolors.FAIL + emoji.emojize(":warning: RED messages mean u skid like shit!") + bcolors.ENDC) #This is an example red text, ooooo
time.sleep(3)
    ...
    class SpamBotnet(KikClientCallback):
        ...
        def on_chat_message_received(self, chat_message: chatting.IncomingChatMessage): #This is triggered when a PM is recieved
            ...
            if mssg == "ping": #command to check if bot is online
                self.client.send_chat_message(JID, "Ping this nut sack motherfucker") #Replies "pong". Pretty simple
            elif mssg == "amigo": #command to add the bot as a friend
                self.client.add_friend(JID) #This is a friend attribution request
                self.client.send_chat_message(JID, "Orale carnal.")
                ...
                try: #This attempts the gif query once first to check if its valid
                    ...
                    self.client.send_chat_message(JID, "I fukt up and lost your gif for the query" + gif_query + "\"!")
                    ...
                    if " w/ " in mssg: #This checks if "w/" was in the string and continues if so
                        if mssg.startswith("spam gif"):
                            ...
                            if "groups" in jid_to_spam: #Checks if the JID they gave is actually a GJID and warns them if so.
                                ...
                                print("I am shitting on the user \"" + jid_to_spam + "\" with the message \"" + emoji.emojize(
                                    message_to_spam) + "\"!")
